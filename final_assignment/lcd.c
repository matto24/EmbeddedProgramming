...
const INT8U LCD_init_sequense[] =
    {
        ...
        0x0F, // Display ON, Cursor OFF, Blink OFF
...
void initLCD(void)
{
  ...
  xQueueLCD = xQueueCreate(QUEUE_LENGTH, sizeof(INT8U));
}
 ...
 *****************************************************************************/
{
  return (xQueueSend(xQueueLCD, &Ch, portMAX_DELAY));
}
 ...
 ******************************************************************************/
{
  ...
  while (1)
  {
    ...
    case LCD_READY:
      ...
      if (uxQueueMessagesWaiting(xQueueLCD))
      {
        if (xSemaphoreTake(xLCDSemaphore, portMAX_DELAY))
        {
          if (xQueueReceive(xQueueLCD, &ch, portMAX_DELAY))
          {
            switch (ch)
            {
            case 0xFF:
